...
const PlayHistoryItem = ({ play }) => (
  <a className="play-history__item" href={play.getIn(["track", "uri"])}>
    <span className="play__info">
      <ListItemCoverImage
        src={play.getIn(["track", "album", "images", 2, "url"])}
      />

      <span className="play__summary">
        <span className="play__artist">
          {play.getIn(["track", "artists", 0, "name"])}
        </span>
        <span className="play__separator">●</span>
        <span className="play__track-name">
          {play.getIn(["track", "name"])}
        </span>
      </span>
    </span>
    <span className="play__time">
      {formatPlayTime(play.getIn(["played_at"]))}
    </span>
  </a>
);
